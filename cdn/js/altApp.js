...
var url = "http://mbbeta.openode.io"
...
function init() {
	socket.emit("allMemes")
	
	socket.on("amRender", (data) => {
		if (data.id == clientID) {
			document.getElementById("allMemes").innerHTML += '<img src="/cdn/memes/' + data.meme + '" style="width:300px;height:200px;visibility:hidden;" id="image">'
		}
	})
	
	socket.on("amComplete", (data) => {
		if (data.id == clientID) {
			var allImage = document.querySelectorAll('#image');
			var i;
			for (i = 0; i < allImage.length; i++) { 
				allImage[i].style.visibility = "visible"
			}
			allImage.style.visibility = "visible"
		}
	})
}
